...
- [ ] `docker compose ps` shows all 5 default services (`sqlserver`,
      `db-init`, `whisper`, `backend`, `nginx`) — `db-init` shows
      `Exited (0)`, all others show `Up` and `(healthy)` where a
      healthcheck exists. (`ollama`/`rag-chatbot` only appear if the
      optional profile was enabled — see below.)
...
## Optional: RAG chatbot (only if this site enabled the `rag-chatbot` profile)

- [ ] `docker compose ps` shows `ollama` and `rag-chatbot` both `Up`, with
      `ollama` `(healthy)`.
- [ ] `docker compose logs rag-chatbot` shows a successful startup data
      refresh (`==> rag-chatbot: data refresh complete.`), not a warning.
- [ ] Log in, open the chatbot panel, ask a question referencing known
      data (real or synthetic depending on this deployment). Confirm an
      answer with sources returns — expect near-instant for exact-count/
      breakdown questions, up to roughly a minute for open-ended questions
      that fall through to the local LLM (CPU-only, no GPU in this
      release — record actual latency on this server's hardware).
- [ ] Confirmed with hospital IT (before enabling this) that eDelphyn
      accepts SQL Server login auth, not Windows-Auth-only — see
      `INSTALL_OFFLINE.md`'s Prerequisites section.
