...
If you see `Temporary failure in name resolution` or anything trying to
reach `huggingface.co`, the model asset wasn't extracted before the stack
started. Fix:
...
container is designed to never need internet; if you ever see a
huggingface.co connection attempt after this fix, `WHISPER_MODEL_PATH` is
probably not mounted correctly — check `compose/docker-compose.yml`'s
`whisper` service volumes.
