...
// Also re-callable on demand: used by Settings → Save/Reload to pick up new
...
app.put('/api/settings/db', requireAdmin, async (req, res) => {
  ...
  // Disabled by default in this deployment — see the RUNTIME_SETTINGS_ENABLED
  // note in settingsStore.js for why. Surfaces as a clean 403 rather than a
  // 500 so the Settings page can show it as an explanation, not a crash.
  try {
    settingsStore.saveSettings({ edelphyn: sanitizeDbSettings(edelphyn) });
  } catch (err) {
    if (err.code === 'RUNTIME_SETTINGS_DISABLED') {
